    ...
    public class ArticleController : VCMSAdminController
    {
        ...
        public ActionResult List(int id, int pageIndex = 1, int pageSize = 20)
        {
            using (DataContext db = new DataContext())
            {
                ...
                var articles = Article.GetByType(id, SiteConfig.LanguageId, flag, pageIndex: pageIndex, pageSize: pageSize, sortOrder: sortOrder, direction: direction, admin: true);
                int count = Article.CountByType(id, SiteConfig.LanguageId, flag, true);

                if (Roles.IsUserInRole("Moderators"))
                {
                    articles = Article.GetByUser(SiteConfig.CurrentUser.UserId, id, SiteConfig.LanguageId, flag, pageIndex: pageIndex, pageSize: pageSize, sortOrder: sortOrder, direction: direction, admin: true);
        ...
        public ActionResult Search(int id, string keyword, int categoryId = 0, int pageIndex = 1, int pageSize = 20)
        {
            using (DataContext db = new DataContext())
            {
                ViewBag.ArticleType = db.ArticleTypes.Include(at => at.Categories.Select(c => c.CategoryDetail)).Include(a => a.ArticleTypeDetail).Single(t => t.Id == id);
                ...
                var articles = Article.Search(id, SiteConfig.LanguageId, keyword, categoryId, pageIndex, pageSize, sortOrder, direction, flag, admin: true);
            ...
            if (ModelState.IsValid)
            {
                ...
                using (DataContext db = new DataContext())
                {
                    ...
                    var vals = from p in type.Properties.Where(p => !p.MultiLanguage && p.PropertyType != PropertyType.MULTICHOICE && p.PropertyType != PropertyType.CHOICE)
                               select new ArticlePropertyValue() { PropertyId = p.Id, Value = Request["P_" + p.Id.ToString()] };
                    ...
                    if (form["relatedArticles"] != null)
                    {
                        var otherVal = model.PropertyValues.FirstOrDefault(v => v.Code.Equals("OTHERARTICLE"));
                        //vals.FirstOrDefault(v => v.Code.Equals("OTHERARTICLE")).Value = form["relatedArticles"];
                        if (otherVal != null)
                        {
                    ...
                    if (type.HasImage && Request.Files.Count > 0)
                    {
                        ...
                        foreach (HttpPostedFileBase file in imagefiles)
                        {
                            ...
                            {
                                if (filenames.Contains(file.FileName.Replace(",","")))
                                {
                                    filenames = filenames.Where(s => !s.Equals(file.FileName.Replace(",", ""), StringComparison.InvariantCultureIgnoreCase)).ToArray();
        ...
        public ActionResult Import(int id)
        {
            using (DataContext db = new DataContext())
            {
                ViewBag.ArticleType = db.ArticleTypes.Include(at => at.Categories.Select(c => c.CategoryDetail)).Include("ArticleTypeDetail").Single(t => t.Id == id);
                return View();
            }
        }
                            ...
                            if (wkb != null)
                            {
                                for (int i = 1; i < wkb.Worksheets.Count + 1; i++)
                                {
                                    ...
                                    if (!currentworksheet.Cells["A5"].Text.Equals("ID") ||
                                        !currentworksheet.Cells["B5"].Text.Equals("Tiêu đề"))
                                        continue;
                                    for (int rowNumber = startRow; rowNumber <= currentworksheet.Dimension.End.Row; rowNumber++)
                                    {
                                        ...
                                        var article = db.Articles.Include(a => a.ArticleDetail).Include(a => a.ArticleType.ArticleTypeDetail).Include(a => a.Prices.Select(p => p.Currency)).Include(a => a.PropertyValues.Select(p => p.Property)).Include(a => a.Discounts).Where(a => a.Id == productid).FirstOrDefault();
        ...
        private ExcelWorksheet CreateHeader(ExcelWorksheet ws1, ArticleType type, Category category)
        {
            ...
            foreach (var lg in SiteConfig.Languages)
            {
                ...
                ws1.Cells[5, rowfix++].Value = html.Locale(string.Format("export_title_lg_{0}", lg.Code.ToLower())).ToHtmlString();
            ...
            if (type.HasNumberProduct)
            {
                ws1.Cells[5, rowfix].Style.HorizontalAlignment = ExcelHorizontalAlignment.Right;
                ws1.Column(colfix).Style.HorizontalAlignment = ExcelHorizontalAlignment.Right;
                ws1.Cells[5, rowfix++].Value = html.Locale("export_number_product").ToHtmlString();
                ws1.Column(colfix++).Width = 10;
            }
            ...
            if (type.HasPrice)
            {
                ws1.Cells[5, rowfix].Style.HorizontalAlignment = ExcelHorizontalAlignment.Right;
                ws1.Column(colfix).Style.HorizontalAlignment = ExcelHorizontalAlignment.Right;
                ws1.Cells[5, rowfix++].Value = html.Locale("export_price_sale").ToHtmlString();
                ws1.Column(colfix++).Width = 15;
            }
        ...
        private ExcelWorksheet GenerateData(ExcelWorksheet ws1, int rowIndexCurrent, ICollection<Article> list)
        {
            ...
            using (DataContext db = new DataContext())
            {
                ...
                foreach (var item in list)
                {
                    ...
                    foreach (var lg in SiteConfig.Languages)
                    {
                        ws1.Cells[rowIndexCurrent, col++].Value = item.ArticleDetail[lg.Id].ArticleName;
                    }
