...
{
    public class APICheckoutV3NL
    {
        public ResponseInfoNL GetUrlCheckoutNL(RequestInfoNL requestContent, string payment_method = "NL")
        {

            requestContent.Payment_method = payment_method;
            String requestinfo = GetParamPostNL(requestContent);
            String result = HttpPostNL(requestinfo);
            result = result.Replace("&", "&amp;");
            XmlDocument dom = new XmlDocument();
            dom.LoadXml(result);
            XmlNodeList root = dom.DocumentElement.ChildNodes;

            ResponseInfoNL resResult = new ResponseInfoNL();
        ...
        public ResponseCheckOrderNL GetTransactionDetail(RequestCheckOrderNL info)
        {
            String request = "";
            request += "function=" + info.Funtion;
            request += "&version=" + info.Version;
            request += "&merchant_id=" + info.Merchant_id;
            request += "&merchant_password=" + CreateMD5HashNL(info.Merchant_password);
            request += "&token=" + info.Token;
            String result = HttpPostNL(request);
            result = result.Replace("&", "&amp;");
            XmlDocument dom = new XmlDocument();
            dom.LoadXml(result);
            XmlNodeList root = dom.DocumentElement.ChildNodes;

            ResponseCheckOrderNL objResult = new ResponseCheckOrderNL();
        ...
        private static String GetParamPostNL(RequestInfoNL info)
        {
        ...
        private static String HttpPostNL(string postData)
        {
            ...
            // Prepare web request...
            HttpWebRequest myRequest = (HttpWebRequest)WebRequest.Create(ConfigurationManager.AppSettings["NganLuong_Url"]);
        ...
        private static String CreateMD5HashNL(string input)
        {
        ...
        private static String GetErrorMessageNL(string _ErrorCode)
        {
            String _Message = "";
            switch (_ErrorCode)
            {
    ...
    public class RequestCheckOrderNL
    {
    ...
    public class ResponseCheckOrderNL
    {
